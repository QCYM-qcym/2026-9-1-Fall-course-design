...
const monthlyRows=count=>Array.from({length:count},(_,i)=>({...row,id:i+1,cityName:'City '+(i+1)}))
const renderedIds=()=>wrapper.findAll('.el-table__body tbody tr').map(tr=>Number(tr.findAll('td')[0].text()))
it('renders only 100 records per page and reaches every record without another GET',async()=>{
 api.fetchForecastRecords.mockResolvedValue(monthlyRows(205));await start()
 expect(wrapper.findAll('[data-test="edit"]')).toHaveLength(100)
 expect(renderedIds()).toEqual(Array.from({length:100},(_,i)=>i+1))
 expect(dom.get('[data-test="page-prev"]').element.disabled).toBe(true)
 await click('page-next');expect(renderedIds()).toEqual(Array.from({length:100},(_,i)=>i+101))
 await click('page-next');expect(renderedIds()).toEqual([201,202,203,204,205])
 expect(dom.get('[data-test="page-next"]').element.disabled).toBe(true)
 await click('page-prev');expect(renderedIds()[0]).toBe(101)
 expect(api.fetchForecastRecords).toHaveBeenCalledTimes(1)
})
it('deleting the only last-page record returns to the previous page with only the normal CRUD refresh',async()=>{
 api.fetchForecastRecords.mockResolvedValue(monthlyRows(201));await start()
 expect(wrapper.findAll('[data-test="edit"]')).toHaveLength(100)
 await click('page-next');await click('page-next');expect(renderedIds()).toEqual([201])
 api.fetchForecastRecords.mockResolvedValue(monthlyRows(200));await click('delete')
 expect(api.deleteForecastRecord).toHaveBeenCalledWith(201)
 expect(renderedIds()).toEqual(Array.from({length:100},(_,i)=>i+101))
 expect(dom.get('[data-test="page-status"]').text()).toContain('2 / 2')
 expect(api.fetchForecastRecords).toHaveBeenCalledTimes(2)
})
it('refresh clamps a shortened or empty list without additional GETs',async()=>{
 api.fetchForecastRecords.mockResolvedValue(monthlyRows(201));await start()
 expect(wrapper.findAll('[data-test="edit"]')).toHaveLength(100)
 await click('page-next');await click('page-next')
 api.fetchForecastRecords.mockResolvedValue(monthlyRows(1));await click('refresh')
 expect(renderedIds()).toEqual([1]);expect(dom.get('[data-test="page-status"]').text()).toContain('1 / 1')
 api.fetchForecastRecords.mockResolvedValue([]);await click('refresh')
 expect(renderedIds()).toEqual([])
 expect(dom.get('[data-test="page-prev"]').element.disabled).toBe(true)
 expect(dom.get('[data-test="page-next"]').element.disabled).toBe(true)
 expect(api.fetchForecastRecords).toHaveBeenCalledTimes(3)
})
