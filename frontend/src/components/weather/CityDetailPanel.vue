...
<template>
  <aside class="city-detail floating-panel" aria-label="城市详情">
    ...
    <template v-if="city">
      <p class="city-value" :class="{ 'no-value': !ready || !Number.isFinite(city.value) }">{{ ready ? formatWeatherValue(city.value) : '等待查询结果' }}<small v-if="ready && Number.isFinite(city.value)">{{ element?.unit }}</small></p>
      <dl>
...
<style scoped>
.city-detail { position: absolute; top: 152px; right: 24px; width: 230px; padding: 14px; max-height: calc(100% - 425px); overflow-y: auto; }
...
@media(max-width: 1000px) { .city-detail { top: 148px; right: 12px; width: 205px; padding: 12px; } .city-value { font-size: 30px; } dl { gap: 6px; } }
</style>
