...
- 初始化范围：山东全部 16 地级市；核心演示济南/青岛，模型 ECMWF/NOAA，要素 T2M（2 米气温，℃）/PRECIP（降水量，mm），约 7 天；不增加新业务表。
...
当前工程尚未初始化，本规则只作为后续目录和版本归档规范，不表示上述工程目录已经创建；实际 versions/ 文档归档可以先于工程存在。
...
<<<<<<< HEAD
- 涉及图表时，验证表格与图表来自同一响应，T2M 趋势/对比为折线图、PRECIP 为柱状图，地图色标与单位正确；另查时间轴零新增请求与写入后缓存失效。
