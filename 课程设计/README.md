...
- [SQL设计说明](03-数据库设计/08-SQL设计说明.md)：查看四表 DDL、约束与初始化要求；现有实例不得重复初始化或覆盖。
...
- [工程初始化计划 V2](06-项目管理/05-工程初始化计划.md)：已执行的初始化计划与边界。
...
本项目采用 **Monorepo**：前端、后端、数据库和软件工程文档统一放在一个GitHub仓库中管理。
...
根 frontend/、backend/、database/ 保存实际工程与脚本；课程设计/ 保存文档、Canvas 和上下文，versions/ 保存阶段档案，不复制源码。
...
当前只做 SYSTEM-FINALIZE-1 最终验证；通过前不宣称 v1.0 验收完成，不自动进入新阶段。启动与待验收说明见 [RESUME](docs/context/RESUME.md)。
